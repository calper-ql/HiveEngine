...
namespace HiveEngine::Renderer {
    ...
    void Context::create_depth_resources() {
        ...
        auto res = vmaCreateImage(allocator, &imageInfo, &textureCreateInfo,
                                  &depth_image, &depth_image_allocation, nullptr);

        if(res != VK_SUCCESS){
            spdlog::error("Context depth image create error: " + std::to_string(res));
            process_error();
        }
        ...
        if (vkCreateImageView(get_device(), &viewInfo, nullptr, &depth_image_view) != VK_SUCCESS) {
            throw std::runtime_error("failed to create texture image view!");
        }

        transition_image_layout(depth_image, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
    }

    VkImageView Context::get_depth_image_view() {
